...
// Lead form -> WhatsApp
const leadForm = /** @type {HTMLFormElement | null} */ (
 ...
 */
function readWaLabels(form) {
  return {
    hello: form.dataset.waHello || "Hello! I want an estimate.",
    name: form.dataset.waName || "Name",
    city: form.dataset.waCity || "City",
    service: form.dataset.waService || "Service",
    area: form.dataset.waArea || "Area",
    start: form.dataset.waStart || "Start",
    comment: form.dataset.waComment || "Comment",
    media: form.dataset.waMedia || "I can send photos/videos.",
    dash: form.dataset.dash || "-",
  };
}
...
if (leadForm) {
  leadForm.addEventListener("submit", (e) => {
    ...
    const wa = readWaLabels(leadForm);

    const name = clean(formData.get("name")) || wa.dash;
    const city = clean(formData.get("city")) || wa.dash;
    const area = clean(formData.get("area")) || wa.dash;
    const time = clean(formData.get("time")) || wa.dash;
    const comment = clean(formData.get("comment")) || wa.dash;
    const service = selectedServiceLabel(leadForm) || wa.dash;
    ...
    const url = `https://wa.me/48784787014?text=${encodeURIComponent(msg)}`;
